...
SeftFlow consists of the frontend, backend API, background worker, PostgreSQL, Redis, and local file storage:
...
SeftFlow separates model capabilities by modality.
...
- `openai_images` (Images API `images.generate` / `images.edit` compatible interface; it does not use Responses `previous_response_id`, and SeftFlow explicitly sends the selected base image plus references for iterative image sessions)
- `google_gemini_image` (Google Gemini native `generateContent` image API through the official `google-genai` SDK; SeftFlow explicitly sends the selected base image plus references for iterative image sessions)
