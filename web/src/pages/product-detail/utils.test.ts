...
describe("product-detail utils", () => {
  ...
  it("reads workflow run retry metadata and includes the previous failure in active retry text", () => {
    const run = workflowRun({
      progress_metadata: {
        last_failure_reason: "Image provider rejected this content",
    ...
    expect(
      workflowRunQueueText(
        ...
        stubT({
          ...
          "detail.runQueuedText": "Queued at position {position}, {ahead} ahead; {active}/{max} active globally.",
        }),
      ),
    ).toBe("Last failure: Image provider rejected this content. Retrying. Queued at position 1, 0 ahead; 1/3 active globally.");
    expect(
      workflowRetryHintLabel(
        "revise_input",
        stubT({
          "detail.retryHint.reviseInput": "Adjust input and run again",
        }),
      ),
    ).toBe("Adjust input and run again");
  });
  ...
  it("formats active node and node-run status labels through shared helpers", () => {
    expect(workflowNodeActivityText({ ...baseNode, node_type: "copy_generation", status: "queued" })).toBe("Node added to run queue");
    expect(workflowNodeActivityText({ ...baseNode, node_type: "copy_generation", status: "running" })).toBe("Node is running");
    expect(workflowNodeActivityText({ ...baseNode, node_type: "copy_generation", status: "idle" })).toBe("");
    expect(workflowNodeRunStatusLabel("failed")).toBe("Failed");
    expect(workflowNodeRunStatusLabel("cancelled")).toBe("Cancelled");
    expect(
      workflowNodeActivityText(
  ...
  it("formats workflow run queue and running text", () => {
    ...
    ).toBe("Queued at position 2, 1 ahead; 3/4 active globally.");
  ...
  it("formats node-run duration and provider summary without exposing raw output", () => {
    ...
    ).toBe("Provider openai-responses / gpt-image-1, status completed, response resp_1");
  ...
  it("labels idle product context nodes as usable static context", () => {
    expect(workflowNodeStatusLabel({ ...baseNode, node_type: "product_context", status: "idle" })).toBe("Available");
    expect(workflowNodeStatusLabel({ ...baseNode, node_type: "image_generation", status: "idle" })).toBe("Not run");
  ...
  it("merges lightweight workflow status without replacing structure or node outputs", () => {
      ...
      nodes: [
        {
          ...
          attempt_count: 2,
          retry_count: 1,
          non_retryable_reason: "Generation failed",
    ...
    expect(merged.nodes[0].failure_reason).toBe("Generation failed");
    ...
    expect(merged.nodes[0].non_retryable_reason).toBe("Generation failed");
