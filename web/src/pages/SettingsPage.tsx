...
function itemsForSection(config: ConfigResponse | undefined, section: SettingsSectionId): ConfigItem[] {
  ...
  if (section === "prompts") {
    return items.filter((item) => item.category === "Prompt templates");
  }
  if (section === "upload") {
    return items.filter((item) => item.category === "Poster and uploads" || item.category === "Image tool parameters");
  }
  if (section === "queue") {
    return items.filter((item) => item.category === "Generation queue");
  }
  if (section === "security") {
    return items.filter((item) => item.category === "Security and operations");
  }
  return [];
}
