...
def download_poster_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Poster file not found") from exc
    if not path.exists():
...
def download_source_asset_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="source image file not found") from exc
    if not path.exists():
