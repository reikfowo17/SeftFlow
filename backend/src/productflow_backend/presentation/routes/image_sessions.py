...
def attach_image_session_asset_to_product_endpoint(
    ...
    message = "Added to product reference images" if payload.target == "reference" else "Set as product main image"
...
def download_image_session_asset_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="session image file not found") from exc
    if not path.exists():
        raise HTTPException(status_code=404, detail="session image file not found")
