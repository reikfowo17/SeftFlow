...
def _single_product_context_node(workflow: ProductWorkflow) -> WorkflowNode:
    ...
    if len(product_context_nodes) != 1:
        raise BusinessValidationError("Template requires a Product info node in the current canvas")
...
def create_workflow_node(
    ...
    *,
    ...
    if node_type == WorkflowNodeType.PRODUCT_CONTEXT and any(
        ...
        raise BusinessValidationError("Product info node already exists")
    node = WorkflowNode(
...
def materialize_node_group_template_to_workflow(
    ...
    *,
    ...
    if workflow is None:
        ...
        raise BusinessValidationError("Open or create a canvas before adding a template")
    ...
    if not insertable_template_nodes:
        raise BusinessValidationError("Template has no nodes that can be added to the current canvas")
...
def duplicate_workflow_node_group(
    ...
    *,
    ...
    if not node_ids:
        raise BusinessValidationError("Please select nodes to copy")
    if len(set(node_ids)) != len(node_ids):
        raise BusinessValidationError("Duplicate copy node ids are not allowed")

    workflow = product_workflow_graph.get_active_workflow(session, product_id)
    if workflow is None:
        product_workflow_graph.get_product_or_raise(session, product_id)
        raise BusinessValidationError("Open or create a canvas before copying nodes")
    ...
    if unknown_node_ids:
        raise BusinessValidationError("Copy includes nodes that do not belong to the current canvas")
...
def update_workflow_copy_set(
    ...
    *,
    ...
    if node.node_type != WorkflowNodeType.COPY_GENERATION:
        raise BusinessValidationError("Only copy nodes can be edited")
    ...
    if not isinstance(copy_set_id, str) or not copy_set_id:
        raise BusinessValidationError("Copy node has not generated copy yet")

    copy_set = session.get(CopySet, copy_set_id)
    if copy_set is None or copy_set.product_id != workflow.product_id:
        raise NotFoundError("Copy version not found")
...
def upload_workflow_node_image(
    ...
    *,
    ...
    """ProductReference image Reference image """
    node = product_workflow_graph.get_node_or_raise(session, node_id)
    if node.node_type != WorkflowNodeType.REFERENCE_IMAGE:
        raise BusinessValidationError("Only reference image nodes can upload images")
    ...
    node.output_json = image_asset_output(
        [asset],
        summary="Reference image replaced",
...
def bind_workflow_node_image(
    ...
    *,
    ...
    SourceAsset  PosterVariant Generate  SourceAsset
    Poster reference SourceAsset
    """
    if bool(source_asset_id) == bool(poster_variant_id):
        raise BusinessValidationError("Please select an image")

    node = product_workflow_graph.get_node_or_raise(session, node_id)
    if node.node_type != WorkflowNodeType.REFERENCE_IMAGE:
        raise BusinessValidationError("Only reference image nodes can be filled")
    workflow = product_workflow_graph.get_workflow_or_raise(session, node.workflow_id)
    ...
    if source_asset_id:
        ...
        if asset.kind != SourceAssetKind.REFERENCE_IMAGE:
            raise BusinessValidationError("Only reference image assets can be bound")
        ...
        if asset is None:
            ...
            try:
                ...
                raise BusinessValidationError("Poster file not found") from exc
...
def create_workflow_edge(
    ...
    *,
    ...
    if source_node_id == target_node_id:
        raise BusinessValidationError("Workflow edge cannot connect a node to itself")
    if source_node_id not in nodes or target_node_id not in nodes:
        raise BusinessValidationError("Workflow edge nodes must belong to the current product")
    edge = WorkflowEdge(
        workflow_id=workflow.id,
...
def delete_workflow_node(session: Session, *, node_id: str) -> ProductWorkflow:
    ...
    if (
        ...
        raise BusinessValidationError("Currently running, please retry deletion later")
