...
def user_canvas_template_to_canvas_template(row: UserCanvasTemplate) -> CanvasTemplate:
    ...
    return CanvasTemplate(
        ...
        scenario=CanvasTemplateScenarioMetadata(
            ...
            title="User template",
            description="User-saved node group",
            ecommerce_stage="custom",
            tags=("User template",),
        ),
        nodes=tuple(
            CanvasTemplateNodeSpec(
...
def create_user_canvas_template_from_workflow_nodes(
    ...
    *,
    ...
    if not clean_title:
        raise BusinessValidationError("Template name must not be empty")
    if not node_ids:
        raise BusinessValidationError("Please select nodes to save")
    if len(set(node_ids)) != len(node_ids):
        raise BusinessValidationError("Saved template nodes must be unique")
    ...
    if workflow is None:
        ...
        raise BusinessValidationError("Open or create a canvas before saving a template")
    ...
    if unknown_node_ids:
        raise BusinessValidationError("Saved template includes nodes not in the current canvas")
    ...
    if any(node.node_type == WorkflowNodeType.PRODUCT_CONTEXT for node in selected_nodes):
        raise BusinessValidationError("Node group templates cannot include the product info node")
...
def rename_user_canvas_template(
    ...
    *,
    ...
    if template.archived_at is not None:
        raise NotFoundError("User template not found")
...
def _parse_template_payload(row: UserCanvasTemplate) -> UserCanvasTemplatePayload:
    if row.kind != "node_group" or row.schema_version != USER_TEMPLATE_SCHEMA_VERSION:
        raise BusinessValidationError("User template version is not supported")
    ...
    if isinstance(value, dict):
        ...
        for key, nested_value in value.items():
            if not isinstance(key, str):
                raise BusinessValidationError("Template configuration contains non-reusable artifact data")
