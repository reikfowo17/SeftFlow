...
WORKFLOW_IMAGE_GENERATION_FAILURE = "Image generation failed, please retry later"
WORKFLOW_IMAGE_GENERATION_TIMEOUT_FAILURE = "Image generationtimeout, please retry later"
...
def execute_workflow_image_generation(
    ...
    *,
    ...
    if not downstream_nodes:
        raise BusinessValidationError("Connect the image generation node to at least one image or reference image node before running")
    ...
    return {
        ...
        "summary": f"filled {len(filled_reference_node_ids)} reference images",
    }
...
def generate_workflow_images_concurrently(
    *,
    ...
    def raise_provider_error(exc: Exception, *, image_provider: ImageProvider, target_index: int) -> None:
        logger.warning(
            (
                "Workflow image provider generation failed: target_index=%s provider=%s model=%s "
