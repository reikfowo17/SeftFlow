...
def resolve_product_creation_canvas_template(canvas_template_key: str | None) -> CanvasTemplate | None:
    ...
    if template.kind != "full_canvas":
        raise BusinessValidationError("Product creation only supports full canvas templates; add node-group templates inside the canvas")
    return template
...
def materialize_product_workflow_from_template(
    ...
    *,
    ...
    if active_workflow_id is not None:
        raise BusinessValidationError("Product already has a campaign canvas")
    ...
    *,
    ...
    for connection in template.default_external_connections:
        ...
        if source_node is None:
            raise BusinessValidationError("Node group templates require a product info node in the current canvas")
        session.add(
            WorkflowEdge(
                workflow_id=workflow.id,
