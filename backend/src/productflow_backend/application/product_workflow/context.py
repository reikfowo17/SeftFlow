...
def poster_kind_from_config(config: dict[str, Any]) -> PosterKind:
    ...
    try:
        ...
        raise BusinessValidationError("image generation node contains unsupported image type") from exc
...
def image_size_from_config(config: dict[str, Any]) -> str | None:
    ...
    if raw is None or (isinstance(raw, str) and not raw.strip()):
        return None
    return normalize_image_generation_size(raw, label="image generation size")
    ...
    for candidate in candidates:
        ...
        if candidate.node_type in {WorkflowNodeType.REFERENCE_IMAGE, WorkflowNodeType.IMAGE_GENERATION}:
            ...
            for image in images:
                ...
                role = str(image.get("role") or "Reference image")
                filename = str(image.get("filename") or "")
                suffix = f", file: {filename}" if filename else ""
                context.append_text(node=candidate, label="reference image", text=f"Reference image: {label} (role: {role}{suffix}) ")
        if candidate.node_type == WorkflowNodeType.COPY_GENERATION:
            ...
            if product_source_assets:
                image_labels = ", ".join(asset.original_filename or "original product image" for asset in product_source_assets)
                context.append_text(node=candidate, label="product image", text=f"product image: {image_labels}")
            ...
            product_parts = [
                ...
                f"Category: {product_context['category']}" if product_context["category"] else "",
            ...
            if summary:
                context.append_text(node=candidate, label="summary", text=summary)
