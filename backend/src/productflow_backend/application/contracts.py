...
def _normalize_ai_scalar_text(value: Any, *, field_name: str) -> Any:
    ...
    for item in value:
        if not isinstance(item, str):
            raise ValueError(f"Each item in {field_name} must be a string.")
        text = item.strip()
        if not text:
            raise ValueError(f"Items in {field_name} cannot be empty.")
        parts.append(text)
    return "、".join(parts)
...
class CopySlotRequest(BaseModel):
    ...
    def validate_required_text(cls, value: str) -> str:
        value = value.strip()
        if not value:
            raise ValueError("The copy slot key and label cannot be empty.")
        return value
...
class CopyBlock(BaseModel):
    ...
    def validate_text(cls, value: str) -> str:
        value = value.strip()
        if not value:
            raise ValueError("The copy block body cannot be empty.")
        return value
...
class CopySection(BaseModel):
    ...
    def validate_section_content(self) -> CopySection:
        if not (self.title or self.body or self.items or self.visual_hint):
            raise ValueError("The copy section cannot be empty.")
        return self
...
class CopyPayloadV2(BaseModel):
    ...
    def validate_summary(cls, value: str) -> str:
        value = value.strip()
        if not value:
            raise ValueError("The copy summary cannot be empty.")
        return value
