...
def get_log_file_path(settings: Settings | None = None) -> Path:
    """Return the effective persistent SeftFlow log file path."""
...
def configure_logging(settings: Settings | None = None) -> None:
    ...
    propagate to the root logger, so SeftFlow mirrors them to the same rotating file handler while leaving their
    ...
    logging.getLogger(__name__).info("Persistent logging enabled: file=%s", log_file.resolve())
...
def cleanup_old_logs(settings: Settings | None = None) -> int:
    ...
    for path in log_dir.glob("*.log*"):
        ...
        try:
            ...
            logging.getLogger(__name__).exception("failed to clean log file: path=%s", path)
    logging.getLogger(__name__).info(
        "Log cleanup done: dir=%s deleted=%s retention_days=%s",
        log_dir,
        deleted,
        retention_days,
    )
...
class _SeftFlowFormatter(logging.Formatter):
...
def _ensure_shared_file_handler(
    ...
    """Install one shared SeftFlow file handler for all persistent log mirrors."""
