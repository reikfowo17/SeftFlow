        ...
        notes.append(
            {
                ...
                "message": f"Gemini uses the closest available aspect ratio {aspect_ratio}. ",
                "requested_size": size,
...
class GoogleGeminiImageClient:
    ...
    def _client(self) -> genai.Client:
        ...
        if self.base_url:
            raise RuntimeError("Google Gemini Providercustom values are not yet supported Base URL")
        return genai.Client(
    ...
    def generate_image(
        ...
        *,
        ...
        try:
            ...
            logger.error("Google Gemini image provider request failed: %s", exc, exc_info=True)
