...
DELETION_DISABLED_DETAIL = "Deletion is disabled. Contact the administrator."
...
def test_product_and_image_session_delete_are_disabled_by_default_and_preserve_records_and_files(
    ...
    created_product = client.post(
        "/api/products",
        data={"name": "default deletion blocked product"},
        files=[
    ...
    created_session = client.post("/api/image-sessions", json={"title": "session deletion disabled by default"})
