...
def test_auth_session_required(configured_env: Path) -> None:
    ...
    assert wrong_key.json()["detail"] == "Invalid administrator access key"
...
def test_admin_access_can_be_disabled_and_re_enabled(configured_env: Path) -> None:
    ...
    assert locked_settings.json()["detail"] == "Please unlock system configuration first"
...
def test_settings_api_requires_secondary_unlock(configured_env: Path) -> None:
    ...
    assert locked_config.json()["detail"] == "Please unlock system configuration first"

    wrong = client.post("/api/settings/unlock", json={"token": "wrong-token"})
    assert wrong.status_code == 401
    assert wrong.json()["detail"] == "Settings unlock token is invalid"
...
def test_settings_unlock_does_not_bypass_missing_token_after_env_change(
    ...
    assert config.json()["detail"] == "Settings unlock token is not configured; contact the administrator"
...
def test_settings_api_persists_database_overrides(configured_env: Path) -> None:
    ...
    assert initial_items["image_session_stale_running_after_minutes"]["category"] == "Generation queue"
    ...
    assert initial_items["workflow_image_generation_provider_timeout_seconds"]["category"] == "Generation queue"
    ...
    assert initial_items["admin_access_required"]["category"] == "Security and operations"
    assert initial_items["deletion_enabled"]["value"] is False
    assert initial_items["deletion_enabled"]["category"] == "Security and operations"
    ...
    assert "must not be less than 1" in invalid_timeout.json()["detail"]
    ...
    assert "must not be less than 1" in invalid_workflow_timeout.json()["detail"]
    ...
    assert "Unknown configuration key: image_provider_kind" in legacy_provider_update.json()["detail"]
...
def test_settings_export_includes_migratable_runtime_config_provider_secrets_and_excludes_env_only(
    ...
    assert payload["metadata"]["app"] == "SeftFlow"
...
def test_settings_import_preview_and_commit_replaces_runtime_and_provider_config(configured_env: Path) -> None:
    ...
    assert preview.json() == {
        ...
        "provider_profile_names": ["import gateway"],
...
def test_settings_import_rejects_unknown_version_and_rolls_back_invalid_bindings(configured_env: Path) -> None:
    ...
    assert rejected_version.json()["detail"] == "Configuration file version not supported"
    ...
    assert "Provider not found" in rejected_import.json()["detail"]
...
def test_provider_config_api_masks_keys_preserves_blank_update_and_validates_bindings(configured_env: Path) -> None:
    ...
    assert "does not support the current interface capability" in invalid_binding.json()["detail"]
    ...
    assert "Copy product understanding model is not configured" in missing_text_model.json()["detail"]
    ...
    assert "image model not configured" in missing_image_model.json()["detail"]
    ...
    assert "Image Responses background mode is not configured" in missing_responses_background.json()["detail"]
    ...
    assert "cannot remove the current interface capability" in remove_active_capability.json()["detail"]
    ...
    assert "cannot be disabled" in disable_active_profile.json()["detail"]

    archive_active = client.delete(f"/api/settings/provider-profiles/{profile_id}")
    assert archive_active.status_code == 400
    assert "still in use by copy or image configuration and cannot be archived" in archive_active.json()["detail"]
...
def test_provider_config_supports_google_gemini_profiles_bindings_and_import(configured_env: Path) -> None:
    ...
    assert "does not yet support custom Base URL" in rejected_base_url.json()["detail"]
...
def test_resolvers_reject_missing_models_instead_of_using_legacy_defaults(
    ...
    with pytest.raises(RuntimeError) as text_error:
        resolve_text_provider_config()
    assert "Copy product understanding model is not configured" in str(text_error.value)

    with pytest.raises(RuntimeError) as image_error:
        resolve_image_provider_config()
    assert "image model not configured" in str(image_error.value)
...
def test_images_api_runtime_options_normalize_and_validate_without_provider_key(configured_env: Path) -> None:
    ...
    assert "Unknown configuration key: image_images_quality" in str(error.value)
...
def test_settings_api_accepts_and_validates_optional_image_tool_fields(configured_env: Path) -> None:
    ...
    assert initial_items["image_tool_quality"]["category"] == "Image tool parameters"
    ...
    assert "Allowed tool fields contain unsupported fields" in invalid_field.json()["detail"]
...
def test_prompt_settings_api_accepts_rejects_and_resets(configured_env: Path) -> None:
    ...
    assert initial.status_code == 200
    initial_items = {item["key"]: item for item in initial.json()["items"]}
    assert initial_items["prompt_copy_system"]["category"] == "Prompt"
    ...
    assert "Taobao e-commerce copy assistant" in initial_items["prompt_copy_system"]["value"]
    assert initial_items["prompt_poster_image_edit_template"]["category"] == "Prompt"
    assert initial_items["prompt_poster_image_edit_template"]["input_type"] == "textarea"
    assert "explicitly connected upstream context" in initial_items["prompt_poster_image_edit_template"]["value"]
    assert initial_items["prompt_poster_image_reference_policy"]["category"] == "Prompt"
    assert initial_items["prompt_poster_image_reference_policy"]["input_type"] == "textarea"
    assert "use the product/subject in the input images as the visual baseline" in initial_items["prompt_poster_image_reference_policy"]["value"]

    updated = client.patch("/api/settings", json={"values": {"prompt_copy_system": "custom copy system prompt"}})
    assert updated.status_code == 200
    updated_items = {item["key"]: item for item in updated.json()["items"]}
    assert updated_items["prompt_copy_system"]["value"] == "custom copy system prompt"
    ...
    assert "must not be empty" in empty.json()["detail"]
    ...
    assert "Taobao e-commerce copy assistant" in reset_items["prompt_copy_system"]["value"]
...
def test_settings_api_rejects_invalid_effective_config(configured_env: Path) -> None:
    ...
    assert "Main image size (compat default)" in response.json()["detail"]
...
def test_settings_api_normalizes_custom_image_sizes_for_generation(configured_env: Path) -> None:
    ...
    generated = client.post(
        ...
        json={"prompt": "generate oneimagecustom sizeproduct image", "size": "512x512"},
    )
...
def test_image_generation_max_dimension_runtime_config_controls_size_bounds(configured_env: Path) -> None:
    ...
    created = client.post("/api/image-sessions", json={"title": "runtime size upper bound"})
    assert created.status_code == 201
    generated = client.post(
        f"/api/image-sessions/{created.json()['id']}/generate",
        json={"prompt": "Size should be calibrated to the runtime upper bound", "size": "3840x2160"},
    )
    assert generated.status_code == 202
    ...
    assert "must not be less than 512" in rejected.json()["detail"]
