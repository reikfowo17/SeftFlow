...
def test_selected_node_plan_rejects_broken_target_without_database_session() -> None:
    ...
    with pytest.raises(ValueError, match="Workflow edge references a non-existent node"):
...
def test_selected_node_plan_rejects_cycle_without_database_session() -> None:
    ...
    with pytest.raises(ValueError, match="Workflow must not contain cyclic dependencies"):
