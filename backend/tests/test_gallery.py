...
def test_generated_image_can_be_saved_to_gallery_idempotently(configured_env: Path, db_session) -> None:
    ...
    product = client.post(
        "/api/products",
        data={"name": "gallery product"},
        files={"image": ("source.png", _make_demo_image_bytes(), "image/png")},
    )
    assert product.status_code == 201
    product_id = product.json()["id"]

    created_session = client.post("/api/image-sessions", json={"product_id": product_id, "title": "gallery session"})
    assert created_session.status_code == 201
    session_id = created_session.json()["id"]

    generated = client.post(
        f"/api/image-sessions/{session_id}/generate",
        json={"prompt": "an image for the gallery", "size": "1024x1024", "generation_count": 2},
    )
    ...
    assert payload["image_session_title"] == "gallery session"
    assert payload["product_id"] == product_id
    assert payload["product_name"] == "gallery product"
    assert payload["prompt"] == "an image for the gallery"
...
def test_gallery_rejects_non_generated_session_assets(configured_env: Path) -> None:
    ...
    client = TestClient(app)
    _login(client)
    created_session = client.post("/api/image-sessions", json={"title": "reference imagesession"})
    assert created_session.status_code == 201
    session_id = created_session.json()["id"]
    ...
    assert saved.json()["detail"] == "Only generation results can be saved to the gallery"


def test_gallery_rejects_generated_asset_without_round(configured_env: Path, db_session) -> None:
    session = ImageSession(title="isolatedgenerated image")
    ...
    assert saved.json()["detail"] == "Generation record not found"
...
def test_gallery_save_handles_integrity_race(
    ...
    session = ImageSession(title="concurrent savesession")
    db_session.add(session)
    db_session.flush()
    asset = ImageSessionAsset(
        session_id=session.id,
