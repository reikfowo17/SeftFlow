...
def test_legacy_copy_fields_migrate_to_structured_payload_and_drop_columns(
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            ...
            {"id": "product-1", "name": "migrateproduct", "now": now},
        )
        connection.execute(
            sa.text(
                """
            ...
            ),
            {
                ...
                "status": "draft",
                "text_value": "old title",
                "points_value": '["selling point one", "selling point two"]',
                "headline_value": "old poster title",
                "action_value": "buy now",
                "model_text_value": "model old title",
                "model_points_value": '["model selling point"]',
                "model_headline_value": "model poster title",
                "model_action_value": "model CTA",
                "provider_name": "test",
                "model_name": "test",
                "prompt_version": "test",
                "now": now,
    ...
    assert structured_payload["summary"] == "old poster title"
    ...
    assert model_structured_payload["summary"] == "model poster title"
    engine.dispose()
    ...
    assert row[LEGACY_COPY_COLUMNS[0]] == "old title"
    assert json.loads(row[LEGACY_COPY_COLUMNS[1]]) == ["selling point one", "selling point two"]
    assert row[LEGACY_COPY_COLUMNS[2]] == "old poster title"
    assert row[LEGACY_COPY_COLUMNS[3]] == "buy now"
    assert row[MODEL_LEGACY_COPY_COLUMNS[0]] == "model old title"
    assert json.loads(row[MODEL_LEGACY_COPY_COLUMNS[1]]) == ["model selling point"]
    assert row[MODEL_LEGACY_COPY_COLUMNS[2]] == "model poster title"
    assert row[MODEL_LEGACY_COPY_COLUMNS[3]] == "model CTA"
    engine.dispose()
    get_settings.cache_clear()
    ...
    with engine.begin() as connection:
        connection.execute(
            sa.text(
                ...
                "VALUES ('product-1', 'retry migrationproduct', :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
            sa.text(
                ...
                "VALUES ('workflow-1', 'product-1', 'migrate workflow', 1, :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
...
def test_workflow_run_progress_metadata_migration_supports_sqlite(tmp_path: Path, monkeypatch) -> None:
    ...
    with engine.begin() as connection:
        connection.execute(
            sa.text(
                "INSERT INTO products (id, name, created_at, updated_at) "
                "VALUES ('product-1', 'progress migrationproduct', :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
    ...
    with engine.begin() as connection:
        connection.execute(
            ...
            {"metadata": json.dumps({"last_failure_reason": "last failure"})},
        )
        metadata = connection.execute(
            sa.text("SELECT progress_metadata FROM workflow_runs WHERE id = 'run-1'")
        ).scalar_one()
    assert json.loads(metadata)["last_failure_reason"] == "last failure"
    ...
    with engine.begin() as connection:
        connection.execute(
            sa.text(
                ...
                "VALUES ('session-1', 'migration session', :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
            sa.text(
                ...
                "VALUES ('task-1', 'session-1', 'running', 'old task', '1024x1024', 2, :now, 1, 1)"
            ),
            {"now": now},
        )
        connection.execute(
            sa.text(
                "INSERT INTO image_session_generation_tasks "
                "(id, session_id, status, prompt, size, generation_count, created_at, attempts, is_retryable) "
                "VALUES ('failed-task-1', 'session-1', 'failed', 'oldfailedtask', '1024x1024', 4, :now, 1, 0)"
            ),
            {"now": now},
        )
...
def test_alembic_upgrade_removes_legacy_workflow_nodes(tmp_path: Path, monkeypatch) -> None:
    ...
    with engine.begin() as connection:
        connection.execute(
            sa.text(
                "INSERT INTO products (id, name, created_at, updated_at) "
                "VALUES ('product-1', 'old workflowproduct', :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
            sa.text(
                "INSERT INTO product_workflows (id, product_id, title, active, created_at, updated_at) "
                "VALUES ('workflow-1', 'product-1', 'old workflow', 1, :now, :now)"
            ),
            {"now": now},
        )
...
def test_disjoint_workflow_node_run_migration_constraints_support_sqlite(
    ...
    with engine.begin() as connection:
        connection.execute(
            sa.text(
                "INSERT INTO products (id, name, created_at, updated_at) "
                "VALUES ('product-1', 'nodeparallel migrationproduct', :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
            sa.text(
                "INSERT INTO product_workflows (id, product_id, title, active, created_at, updated_at) "
                "VALUES ('workflow-1', 'product-1', 'nodeparallelmigrate workflow', 1, :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
            sa.text(
                ...
                "VALUES "
                "('node-1', 'workflow-1', 'copy_generation', 'copy', 0, 0, '{}', 'queued', :now, :now), "
                "('node-2', 'workflow-1', 'image_generation', 'image generation', 100, 0, '{}', 'queued', :now, :now)"
            ),
            {"now": now},
        )
        connection.execute(
