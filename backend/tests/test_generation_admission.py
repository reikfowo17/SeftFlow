...
def test_generation_cap_accepts_and_queues_workflow_run_creation(
    ...
    busy_product = _create_product(db_session, "occupy concurrencyproduct")
    ...
    db_session.commit()
    workflow_target = _create_product(db_session, "workflow rate limitedproduct")
...
def test_generation_cap_accepts_and_queues_image_session_generation_task_creation(
    ...
    image_session = create_image_session(db_session, product_id=None, title="sync occupancy session")
    ...
    created = client.post("/api/image-sessions", json={"title": "sync rate limit"})
    assert created.status_code == 201

    generated = client.post(
        f"/api/image-sessions/{created.json()['id']}/generate",
        json={"prompt": "this timeshould be blocked by concurrency limit", "size": "1024x1024"},
    )
...
def test_active_generation_task_count_includes_image_session_generation_tasks(
    ...
    image_session = create_image_session(db_session, product_id=None, title="concurrency count")
    result = create_image_session_generation_task(
        db_session,
        image_session_id=image_session.id,
        prompt="occupy continuousimage generationtask",
        size="1024x1024",
    )
...
def test_generation_queue_overview_and_positions_include_durable_tasks(
    ...
    image_session = create_image_session(db_session, product_id=None, title="queue session")
    second_image_session = create_image_session(db_session, product_id=None, title="queue session 2")
    first = create_image_session_generation_task(
        db_session,
        image_session_id=image_session.id,
        prompt="first continuousimage generationtask",
        size="1024x1024",
    ).task
    second = create_image_session_generation_task(
        db_session,
        image_session_id=second_image_session.id,
        prompt="second continuousimage generationtask",
        size="1024x1024",
    ).task
...
def test_generation_queue_overview_endpoint_returns_public_snapshot(
    ...
    image_session = create_image_session(db_session, product_id=None, title="queue API session")
    running = create_image_session_generation_task(
        db_session,
        image_session_id=image_session.id,
        prompt="run continuousimage generationtask",
        size="1024x1024",
    ).task
    running.status = JobStatus.RUNNING
    db_session.commit()
