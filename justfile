...
# AI Agent (Google ADK) + MCP server
mcp-serve:
    bash scripts/with_dev_env.sh uv run --directory backend python -m productflow_backend.mcp_server

agent-run: backend-run

skill-install:
    bash -lc 'dest="${CODEX_HOME:-$HOME/.codex}/skills/seftflow"; mkdir -p "$dest/scripts"; cp skills/seftflow/SKILL.md "$dest/SKILL.md"; cp skills/seftflow/scripts/run_mcp.py "$dest/scripts/run_mcp.py"; echo "Installed SeftFlow skill to $dest"'
